...
export async function renderCarouselSlide(app, p, i, canvas, lang = 'pt') { await ensureFonts(); const spec = await buildSpec(app, p, lang); const s = ((lang === 'es' && p.carousel_es?.slides?.length) ? p.carousel_es.slides : p.carousel.slides)[i]; const R = activeRules(); return renderSlide(canvas, { ...spec, slide: { ...s, index: i, total: p.carousel.slides.length }, cta: p.content_json.caption?.cta || '', tagline: lang === 'es' ? (R.newsletter.tagline_es || 'Ventas para quienes influyen en las ventas') : R.newsletter.tagline }); }
