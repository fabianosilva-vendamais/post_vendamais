...
function label(ctx, text, x, y, color) { ctx.font = `600 22px ${FONT}, Arial, sans-serif`; ctx.letterSpacing = '2px'; ctx.fillStyle = color; ctx.fillText(String(text || '').toUpperCase(), x, y); ctx.letterSpacing = '0px'; }
...
export function render(canvas, spec) {
  ...
  if (t === 'T01') {
    ...
    const sup = fit(ctx, spec.support, inner - 32, { max: 28, min: 24, weight: 400, maxLines: 3, lh: 1.4 });
    ...
    if (spec.support) { const sup = fit(ctx, spec.support, inner, { max: 26, min: 22, weight: 400, maxLines: 3, lh: 1.4 }); ctx.font = `400 ${sup.size}px ${FONT}, Arial, sans-serif`; drawLines(ctx, sup.lines, M, y + 16, sup.lineH, C.gray); }
    meta.headlineFontPx = hl.size;
  } else if (t === 'T03') {
    ...
    if (spec.support) { const sup = fit(ctx, spec.support, inner, { max: 26, min: 22, weight: 400, maxLines: 3, lh: 1.4 }); ctx.font = `400 ${sup.size}px ${FONT}, Arial, sans-serif`; drawLines(ctx, sup.lines, M, y + 12, sup.lineH, C.grayl); }
    meta.headlineFontPx = hl.size;
  } else if (t === 'T04') {
    ctx.fillStyle = C.white; ctx.fillRect(0, 0, W, H);
    ...
    if (spec.support) { const sup = fit(ctx, spec.support, pw, { max: 26, min: 22, weight: 400, maxLines: 4, lh: 1.45 }); ctx.font = `400 ${sup.size}px ${FONT}, Arial, sans-serif`; drawLines(ctx, sup.lines, px, y + 20, sup.lineH, C.ink); }
...
export function renderSlide(canvas, spec) {
  ...
  const pager = (color) => { ctx.font = `500 22px ${FONT}, Arial, sans-serif`; ctx.fillStyle = color; ctx.textAlign = 'right'; ctx.fillText(page, W - M, H - M + 8); ctx.textAlign = 'left'; };
  ...
  if (s.role === 'closing') {
    ...
    if (s.body) { const b = fit(ctx, s.body, inner, { max: 28, min: 24, weight: 400, maxLines: 4, lh: 1.45 }); ctx.font = `400 ${b.size}px ${FONT}, Arial, sans-serif`; y = drawLines(ctx, b.lines, M, y + 20, b.lineH, C.grayl); }
    if (spec.cta) { const ct = fit(ctx, spec.cta, inner - 64, { max: 26, min: 22, weight: 600, maxLines: 2, lh: 1.3 }); ctx.font = `600 ${ct.size}px ${FONT}, Arial, sans-serif`; const tw = Math.max(...ct.lines.map(l => ctx.measureText(l).width)); const bw = Math.min(inner, tw + 64), bh = ct.lines.length * ct.lineH + 40; const by = Math.min(H - M - 80 - bh, y + 48); rrect(ctx, M, by, bw, bh, 4); ctx.fillStyle = C.white; ctx.fill(); drawLines(ctx, ct.lines, M + 32, by + 20 + ct.size * 0.85, ct.lineH, C.navy); }
    ctx.font = `500 22px ${FONT}, Arial, sans-serif`; ctx.fillStyle = C.grayl; ctx.fillText(spec.tagline || '', M, H - M + 8); pager('rgba(255,255,255,0.7)'); meta.headlineFontPx = hl.size; return meta;
  }
  ...
  if (s.body) { const b = fit(ctx, s.body, w, { max: 30, min: 24, weight: 400, maxLines: 7, lh: 1.5 }); ctx.font = `400 ${b.size}px ${FONT}, Arial, sans-serif`; drawLines(ctx, b.lines, x, y + 28, b.lineH, C.ink); }
