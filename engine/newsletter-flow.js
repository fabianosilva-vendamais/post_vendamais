...
export async function runQA(app) {
  ...
  const prevAi = v.qa_json?.ai; const applied = Object.keys(v.qa_json?.applied || {}).map(i => prevAi?.issues?.[i]).filter(Boolean);
  const hist = [...(v.qa_json?.applied_history || []), ...applied.map(i => ({ where: i.where, fix: i.fix || i.text }))]; const memory = (hist.length ? ` JÁ RESOLVIDO EM RODADAS ANTERIORES (não repita nem reabra): ${hist.slice(-20).map(i => `${i.where}: ${i.fix}`).join(' | ')}.` : '') + (prevAi && v.score >= R.qa.threshold ? ` A peça já está acima da nota-alvo (${v.score}). Liste no máximo 3 correções, apenas as que mudariam a nota de fato; se não houver, retorne issues vazio.` : '');
